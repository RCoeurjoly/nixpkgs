...
stdenv.mkDerivation {
  ...
  configurePhase = ''
    ...
    cmake -G Ninja \
      ...
      -DPython3_EXECUTABLE=${python313}/bin/python3 \
      -DPython_EXECUTABLE=${python313}/bin/python3 \
  ...
  installPhase = ''
    ...
    if [ -d build/tools/torch-mlir/python_packages ]; then
      mkdir -p "$out/python-packages"
      cp -r build/tools/torch-mlir/python_packages/. "$out/python-packages/"
    fi
